...
class Receivingassets_model extends CI_Model
{
	...
	public function get()
	{
		$users = $this->db->query("SELECT deliveryid,drno,purchase_po.pono,purchase_apr.aprno, receivedate from equipments_receiving left join purchase_apr on equipments_receiving.aprid = purchase_apr.aprid left join purchase_po on equipments_receiving.poid = purchase_po.poid");
	...
	{
		$sql = $this->db->query("SELECT * FROM equipments_receiving left join purchase_apr on equipments_receiving.aprid = purchase_apr.aprid left join purchase_po on equipments_receiving.poid = purchase_po.poid left join suppliers on equipments_receiving.supplierid = suppliers.supplierID WHERE deliveryid='$id'");
	...
	public function addtoasset($deliveryitemsid)
	{
		...
		//getdritems details
		$sqlselect = $this->db->query("SELECT * FROM equipments_receiving_items LEFT JOIN equipments_receiving ON equipments_receiving_items.deliveryid = equipments_receiving.deliveryid LEFT JOIN assets ON equipments_receiving_items.assetid = assets.assetid
WHERE equipments_receiving_items.deliveryitemsid ='$deliveryitemsid'");
		...
		for($ctr=1;$ctr<=$details['qty'];$ctr++){
			$sql = "INSERT INTO equipments(assetid,deliveryid,article,particulars,classification,supplierID,dateacquired,totalcost) VALUES (".$this->db->escape($details['assetid']).",".$this->db->escape($details['deliveryid']).",".$this->db->escape($details['asset_article']).",".$this->db->escape($details['asset_particulars']).",".$this->db->escape($details['asset_classification']).",".$this->db->escape($details['supplierid']).",".$this->db->escape($details['receivedate']).",".$this->db->escape($details['unitprice']).")";
			$this->db->query($sql);
			
			
			
			$sqlselect = $this->db->query("SELECT MAX(equipNo) AS lastid FROM equipments");
			$lastidinserted = $sqlselect->result_array();
			//echo json_encode($lastidinserted[0]);
			$currentid = $lastidinserted[0]['lastid'];
		
			$sql2 = "INSERT INTO equipments_details(equipNo) VALUES (".$this->db->escape($currentid).")";
			$this->db->query($sql2);
